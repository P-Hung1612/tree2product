...
import {CreateYardSchema} from '@application/dtos/CreateYardDTO';

export class CreateYardController extends BaseController {
  // Inject UseCase vào Controller
  constructor(private useCase: CreateYardUseCase) {
    super();
  }
  ...
  public async execute(req: Request, res: Response): Promise<any> {
    try {
      ...
      const validationResult = CreateYardSchema.safeParse(req.body);
      ...
      // 3. Response
      return this.ok(res, {yardId: result.id}); // Chỉ trả về ID hoặc full object tùy nhu cầu
