...
import { ReceiveLoadToTankSchema } from "@application/dtos/ReceiveLoadToTankDTO";

export class ReceiveLoadToTankController extends BaseController {
    ...
    public async execute(req: Request, res: Response): Promise<any> {
        try {
            ...
            // 2. Validate
            const result = ReceiveLoadToTankSchema.safeParse(payload);
            if (!result.success) {
